...
import {
  ImageElement,
  PostDetails,
} from '../../common/interfaces/image-element';
...
export class PosterComponent implements OnInit {
      ...
      this.route.paramMap
        .pipe(
          ...
          tap((post) => {
            ...
            this.postStatus = post.deleted ? 'This image has been deleted'
              : !post.published
              ? 'This image is not published'
              : 'loading';
            this.isUnpublished = !post.published;
  ...
  async getPostById(): Promise<void> {
      ...
      if (!post.deleted) {
        ...
        for (const item of post.data) {
          if (item.image && item.image.imageUrl) {
            try {
              item.image.imageUrl = await this.getImageDataUrl(
                item.image.imageUrl
              );
            } catch (err) {
        ...
        if (this.postDetails.download_counter) {
          this.postStatus =
            'Total Download: ' + this.postDetails.download_counter;
        } else {
    ...
    this.textModal = new bootstrap.Modal(
    ...
    this.imageCropper = new bootstrap.Modal(
  ...
  async drawSVG() {
    if (this.postDetails) {
      ...
      this.renderer.setAttribute(
        svg,
        'viewBox',
        '0 0 ' + (this.postDetails.w || 0) + ' ' + (this.postDetails.h || 0)
      );
      const b = this.renderer.createElement(
        'image',
        'http://www.w3.org/2000/svg'
      );
      ...
      this.postDetails?.data.forEach(async (item, i) => {
        const uniqueId = item.editable
          ? this.dataset[s]?.id || Math.random().toString(36).substr(2, 9)
          : Math.random().toString(36).substr(2, 9);
        switch (true) {
          case !!item.text:
            if (item.text) {
              const t = this.renderer.createElement(
                'text',
                'http://www.w3.org/2000/svg'
              );
              let {
              ...
              if (text) {
                ...
                item.editable &&
                  this.renderer.setStyle(t, 'pointer-events', 'none');
                if (lines.length === 1) {
                  ...
                  lines.forEach((line, index) => {
                    ...
                    const tspanElement = this.renderer.createElement(
                      'tspan',
                      'http://www.w3.org/2000/svg'
                    );
              ...
              if (textShadow.enable) {
                textAttributes['filter'] =
                  `drop-shadow(${textShadow.offsetX}px ${textShadow.offsetY}px ${textShadow.blur}px ${textShadow.color})` ||
                  'none';
              }
              Object.entries(textAttributes).forEach(([key, value]) =>
                this.renderer.setAttribute(t, key, value)
              );
              Object.entries(textStyles).forEach(([key, value]) =>
                this.renderer.setStyle(t, key, value)
              );
              ...
              if (this.dataset[s] == undefined && item.editable) {
                this.dataset.push({
                  id: uniqueId,
                  lang: lang || 'en',
                  value: text || 'Select ' + item.title + '',
                  index: i.toString(),
                  type: type,
                  title: item.title,
          ...
          case !!item.image:
            if (item.image) {
              ...
              switch (shape) {
                case 'circle':
                  element = this.renderer.createElement(
                    'circle',
                    'http://www.w3.org/2000/svg'
                  );
              ...
              if (element !== null) {
                ...
                const imagePattern = this.renderer.createElement(
                  'pattern',
                  'http://www.w3.org/2000/svg'
                );
                ...
                const image = this.renderer.createElement(
                  'image',
                  'http://www.w3.org/2000/svg'
                );
                ...
                const extraRect = this.renderer.createElement(
                  'rect',
                  'http://www.w3.org/2000/svg'
                );
    ...
    for (const key in this.selectData) {
      ...
      const filteredData = this.apiData[key].filter(
        (item) => item.id == data.value
      );
      if (filteredData.length) {
        this.renderer.setValue(
          data.text,
          filteredData[0][data.lang == 'gu' ? 'gu_name' : 'name']
        );
      }
    }
  }
  async buildForm() {
    ...
    this.dataset.forEach((field) => {
      ...
      if (!isNaN(index) && this.postDetails?.data) {
        if (field.type === 'text' || field.type === 'date') {
          ...
          if (textData) {
            const c = this.formData.addControl(
              field.id,
              this.fb.control(field.value, Validators.required)
            );
            this.formData.get(field.id)?.valueChanges.subscribe((v) => {
              textData.text = v;
              field.value = v;
    ...
    for (const key in selectData) {
      ...
      if (
        ...
        selectData[selectData[key].dependency].control.valueChanges.subscribe(
          async (value) => {
            if (value) {
        ...
        console.warn(
          `Dependency "${selectData[key].dependency}" for "${key}" is invalid or not found.`
        );
      }
    }
  ...
  private async setupDependency(
    key: string,
    data: { lang: string; value: string; api: string; dependency: string }
  ) {
      ...
      this.dataset.forEach((field) => {
        ...
        if (!isNaN(index) && this.postDetails?.data) {
          if (field.type === 'text') {
            ...
            const imageData = this.postDetails.data.filter(
              (_, i) => i === index
            )[0]?.image;
            if (imageData) {
        ...
        if (
          ...
          reader.onload = (e) => {
            ...
            if (this.postDetails?.data[i].image) {
              const newWH = this.commonService.calculateWH(
                imageData as ImageElement
              );
              w = newWH.w;
              h = newWH.h;
            }
          ...
          this.toastService.show('Please select image type.', {
            class: 'bg-danger',
            title: 'Invalid Image Type',
          });
        }
      } else {
        this.toastService.show('Please select valid file format.', {
          class: 'bg-danger',
  ...
  async checkDownload(t: string): Promise<boolean> {
    ...
    switch (t) {
      ...
      case 'facebook':
        ...
        const shareDialogUrl = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(
          baseUrl
  ...
  async facebookPhoto() {
    ...
    image.onload = async () => {
      ...
      const blob = this.dataURLtoBlob(this.finalImage);
      // Create a FormData object for the POST request
      const formData = new FormData();
      formData.append('access_token', this.facebookAccessToken);
      formData.append('message', this.postDetails.info);
      formData.append('source', blob, 'image.png');

      await this.http
        .post(`https://graph.facebook.com/v14.0/me/photos`, formData)
        .subscribe(
          async (response) => {
            await this.PS.updateDownloadCounter(this.imgParam).subscribe(
              (post) => {
            ...
            this.postStatus = 'Image posted successfully to Facebook!';
          },
          (error) => {
            console.error('Error posting to Facebook:', error);
            this.postStatus = 'Failed to post image to Facebook.';
          }
        );
      this.loaderService.hide();
    };
    ...
    if (this.selectedIndex !== null && this.postDetails?.data) {
      this.postDetails.data = this.postDetails.data.map((item, index) => {
        if (index === this.selectedIndex && item.text) {
          ...
          if (this.selectedID) {
            const elementToChange = this.elementRef.nativeElement.querySelector(
              `[data-id="${this.selectedID}"]`
            );
            if (elementToChange) {
              const filteredItems = this.dataset.filter(
      ...
      if (fileType.startsWith('image/')) {
        ...
        if (
          ...
          reader.onload = (e) => {
            ...
            const selectedItem = this.dataset.find(
            ...
            if (selectedItem) {
              ...
              if (this.postDetails?.data[index].image) {
                const newWH = this.commonService.calculateWH(
                  this.postDetails?.data[index].image as ImageElement
                );
                w = newWH.w;
                h = newWH.h;
              }
