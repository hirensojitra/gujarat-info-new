...
export class PostDetailService {
  ...
  uploadThumbnail(postId: number, formData: FormData): Observable<any> {
    return this.http.post(`${this.baseUrl}/${postId}/thumbnail`, formData);
  }
  ...
  getTotalPostLength(): Observable<{ totalLength: number }> {
    return this.http.get<{ totalLength: number }>(
      `${this.baseUrl}/post-length`
    );
  }

  getTotalDeletedPostLength(): Observable<{ totalLength: number }> {
    return this.http.get<{ totalLength: number }>(
      `${this.baseUrl}/post-deleted-length`
    );
  }
}
